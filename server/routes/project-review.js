...
function buildIllustrationReview(project) {
    ...
    // chapter-book → use prose/humanized illustration moments
    const sourceChapters = normArr(arts.humanized).length ? normArr(arts.humanized) : normArr(arts.chapters);
    const illustrations = normArr(arts.illustrations);

    sourceChapters.forEach((ch, ci) => {
        const moments = normArr(ch?.illustrationMoments);
        const illCh = illustrations[ci] || {};
        const illSpreads = normArr(illCh.spreads);
            ...
            : [
                {
                    ...
                    illustrationHint: ch?.chapterIllustrationHint || ch?.chapterSummary || '',
                    charactersInScene: [],
