...
//
// FIX CHANGELOG:
// ✅ FONT_COMBO_WATERMARKS cleared — font-combo objects no longer silently dropped from PDF
// ✅ htmlShell() updated to 576×768 (6in×8in at 96dpi) — no more scale(2) crop bug
// ✅ extractContent() filter updated — no longer excludes text matching old watermark strings
// ✅ Both HTML shell paths (htmlShell + main return) use identical 576×768 CSS

const PAGE_W = 750;   // Coordinate space — must match Fabric.js canvas width in the editor
const PAGE_H = 1000;  // Coordinate space — must match Fabric.js canvas height in the editor

// PDF output size in CSS pixels (6in × 8in at 96dpi)
const PDF_CSS_W = 576;
const PDF_CSS_H = 768;

// ─── Font combo watermark filter ─────────────────────────────────────────────
// CLEARED: previously this silently dropped any canvas textbox whose text
// exactly matched the font-combo seed strings ("Bold Editorial", "Modern Clean"
// etc.), making all font-combo–placed objects invisible in the PDF.
// The filter is now empty — all canvas content passes through to the PDF.
...
// originX:'left'/originY:'top' → no adjustment at all
...
// Bake opacity into color so parent opacity doesn't double-apply
...
// ─── Fabric styles[] → HTML <span> segments ──────────────────────────────────
...
function renderText(obj) {
  ...
  // FONT_COMBO_WATERMARKS is now empty — this check never fires,
  // but kept for safety in case the set is re-populated in future.
  ...
  // Separate background div so opacity doesn't double-apply
  let bgHtml = '';
  ...
  if (cropX || cropY) {
    ...
    return `
      <div style="${outerStyle}">
        <img src="${esc(obj.src)}" crossorigin="anonymous" style="${innerStyle}" alt="" loading="eager">
      </div>
    `;
  }
...
// Fabric polygon points are centered around (0,0).
// Shift each point by +width/2, +height/2 so the bounding box starts at 0,0.
...
// ─── HTML shell ───────────────────────────────────────────────────────────────
// FIX: body and .page now match the PDF paper size in CSS pixels (576×768 = 6in×8in at 96dpi).
// Previously used PAGE_W*2 × PAGE_H*2 (1500×2000) with scale(2), which caused
// content to be cropped at 576px when Chrome rendered the 6in PDF page.
// Percentage-based positions (pct()) are still relative to PAGE_W/PAGE_H so
// they resolve proportionally to the 576×768 .page element — no coordinate
...
// FIX: removed FONT_COMBO_WATERMARKS filter from text extraction.
// Previously, objects whose text matched watermark strings were silently excluded,
// causing font-combo–placed text objects to vanish from the PDF.
function extractContent(page, fabricJson) {
  ...
  // FIX: filter only on empty/whitespace text — no watermark exclusion
...
export function renderPageHtml(page, templateId = 'classic') {
  ...
  // "No renderable content" — fall back to thumbnail → imageUrl
  const hasContent = objects.length > 0 || !!bgImageObj;
  if (!hasContent) {
    if (page.thumbnail) return htmlShell(template, '#000000', `<img src="${esc(page.thumbnail)}" crossorigin="anonymous" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" alt="" loading="eager">`);
    if (page.imageUrl)  return htmlShell(template, '#000000', `<img src="${esc(page.imageUrl)}"  crossorigin="anonymous" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" alt="" loading="eager">`);
    return htmlShell(template, fabricJson.background || placeholderBg(page.id), '', []);
  }

  const bgColor = fabricJson.background || template.pageBackground || '#fffef7';
  const parts   = [];

  // fabricJson.backgroundImage as bottom layer
  if (bgImageObj?.src) {
    ...
    if (visibleW >= PAGE_W * 0.9 && visibleH >= PAGE_H * 0.9 && cropX === 0 && cropY === 0) {
      parts.push(`
        <img src="${esc(bgImageObj.src)}" crossorigin="anonymous"
          style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center center;opacity:${bgImageObj.opacity ?? 1};"
          alt="" loading="eager">
      `);
    } else {
      parts.push(`
        <div style="position:absolute;left:${pct(cssLeft,PAGE_W)};top:${pct(cssTop,PAGE_H)};width:${pct(visibleW,PAGE_W)};height:${pct(visibleH,PAGE_H)};overflow:hidden;opacity:${bgImageObj.opacity ?? 1};">
          <img src="${esc(bgImageObj.src)}" crossorigin="anonymous"
            style="position:absolute;left:${pct(-(cropX*scaleX),PAGE_W)};top:${pct(-(cropY*scaleY),PAGE_H)};width:${pct((sourceW+cropX)*scaleX,PAGE_W)};height:${pct((sourceH+cropY)*scaleY,PAGE_H)};max-width:none;max-height:none;"
            alt="" loading="eager">
        </div>
      `);
    }
  ...
  // Sort: __background objects first, then foreground
  const sorted = [
  ...
  for (const obj of sorted) {
    if (!obj) continue;
    if (obj.__background === true && obj.type === 'rect') continue; // skip bg rect placeholder
  ...
  // FIX: 576×768 (6in×8in at 96dpi) — matches Puppeteer viewport.
  // Previously 1500×2000 with scale(2) caused all content to be cropped at 576px.
  // Percentage positions from pct() remain correct because they divide by
  // PAGE_W/PAGE_H (750/1000) and resolve proportionally against the 576×768 .page.
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
// //
// // NOTE: This file is unchanged from the working version.
// // The PDF offset overflow was entirely in puppeteerPdf.js (embedJpg step).
// // No changes are required here.

// const PAGE_W = 750;   // Coordinate space — must match Fabric.js canvas width in the editor
// const PAGE_H = 1000;  // Coordinate space — must match Fabric.js canvas height in the editor
// // The HTML is rendered at this size then CSS-scaled 2× so Puppeteer captures
// // at 1500×2000 = 250 DPI for a 6"×8" PDF (vs. 125 DPI without scaling).

// // ─── Font combo watermark filter ─────────────────────────────────────────────
// const FONT_COMBO_WATERMARKS = new Set(); // filtering disabled — let canvas content pass through
...
//   // Full page / background-like image
//   if (fullBleed && cropX === 0 && cropY === 0) {
//     return `<img
//       src="${esc(obj.src)}"
//       crossorigin="anonymous"
...
//   // If Fabric crop values exist, reproduce them with an inner shifted image
//   if (cropX || cropY) {
//     const outerStyle = [
//       'position:absolute',
//       `left:${pct(cssLeft, PAGE_W)}`,
...
// <style>
//   *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
//   /* body is 2× the page size to contain the scaled-up .page */
//   html,body{width:${PAGE_W * 2}px;height:${PAGE_H * 2}px;overflow:hidden;background:transparent;}
//   :root{--page-bg:${bgColor};}
//   /* .page stays at the fabricJson coordinate size (750×1000) so all
//      pct() positions remain correct, then is scaled 2× to 1500×2000.
//      Puppeteer viewport matches the 2× size → 250 DPI at 6"×8" PDF. */
//   .page{
//     position:relative;
...
//   // "No renderable content" = no objects AND no background image.
//   // A bare background colour is NOT renderable content — if the page never had
//   // objects saved (e.g. corrupted objects:[] or freshly-created page), fall back
//   // to thumbnail → imageUrl rather than rendering a blank coloured square.
//   const hasContent = objects.length > 0 || !!bgImageObj;
//   if (!hasContent) {
//     if (page.thumbnail) return htmlShell(template, '#000000', `<img src="${esc(page.thumbnail)}" crossorigin="anonymous" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" alt="" loading="eager">`);
//     if (page.imageUrl) return htmlShell(template, '#000000', `<img src="${esc(page.imageUrl)}" crossorigin="anonymous" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" alt="" loading="eager">`);
...
// <style>
//   *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
//   html,body{width:576px;height:768px;overflow:hidden;background:transparent;margin:0;padding:0;}
//   :root{--page-bg:${bgColor};}
//  .page{
//     position:relative;
